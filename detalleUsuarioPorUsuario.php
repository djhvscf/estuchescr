<html>
	...
	?>
		...
		<div class="container">
				<?php
					$urlDetalle = "detalleUsuarioPorUsuario.php?" . $queryString;
				...
				<fieldset>
					...
					<?php
								if(array_key_exists('txtNombre',$_POST))
								{
									...
									$id_estado=$rol_usuario;
									$claveActual=$_POST['pwdClaveActual'];
									$claveUs = $gestor->buscarUsuarioContrasena($id_usuario);
									if($claveActual == $claveUs ){
									$gestor->modificarUsuario($id_usuario, $nombre,$apellido_1,$apellido_2,$num_cedula,$nombre_usuario, $contrasena, $id_estado);
									echo "<div id='msj' class='alert alert-success'>El usuario fue modificado.</div>";
									}else{
										echo "<div id='msj' class='alert alert-error' style='display:block'>Clave de usuario actual invalida. Es necesaria para modificar</div>";
									}
								}
								$consulta=$gestor->buscarUsuarioPorID($id_usuario);
							?>
					<div name="divNombre" id="divNombre" class="control-group">
						...
						<div class="controls">
							<?php
								$nombre = $consulta[1];
								echo "<td><input name='txtNombre' type='text' id='txtNombre' value='$nombre' class='input-xlarge' onClick ='' readOnly/></td>";
					...
					<div name="divPrimerApellido" id="divPrimerApellido" class="control-group">
						...
						<div class="controls">
							<?php
								...
								echo "<td><input type='text' name='txtPrimerApellido' id='txtPrimerApellido' value='$apellido_1' class='input-xlarge' onClick='' readOnly/></td>";
							?>
						</div>
					</div>
					<div name="divSegundoApellido" id="divSegundoApellido" class="control-group">
						...
						<div class="controls">
							<?php
								...
								echo "<td><input type='text' name='txtSegundoApellido' id='txtSegundoApellido' value='$apellido_2' class='input-xlarge' onClick='' readOnly/></td>";
							?>
						</div>
					</div>
